...
# join together the species specific measures of negative feedback with species specific functional traits
...
#################### Import Negative Feedback Model Stats ##################

NFmod_stats <- readRDS(here("Outputs", "AverageNegativeSlopebySpecies_m2.rds"))
head(NFmod_stats)
...
NFnames_stats <- left_join(NFmod_stats, NFspp_allnames, by = "SPEC") %>%
...
mapsNF_elton_trophic <- elton %>%
  ...
  left_join(NFnames_stats, ., by="Species3_BirdTree") # join modified elton traits data frame and NFnames_stats data frame

# most species have diet information with certainty = A, meaning high confidence/quality data
dietcert_A <- mapsNF_elton_trophic %>% filter(Diet.Certainty == "A")
print(dietcert_A)

# however, 3 species have diet information that does not have a high confidence associated with it
dietcert_C <- mapsNF_elton_trophic %>% filter(Diet.Certainty == "C")
print(dietcert_C)
...
mapsNF_elton_update <- bind_rows(dietcert_A, diet_update_spp)
...
mapsNF_elton_trophiclevel <- mapsNF_elton_update %>%
  rowwise() %>%
  mutate(AnimalDietPerc = (Diet.Inv + Diet.Vend + Diet.Vect + Diet.Vfish + Diet.Vunk + Diet.Scav),
         TrophicLevel = (AnimalDietPerc/100))

range(mapsNF_elton_trophiclevel$TrophicLevel) # examine range of scores for our species
boxplot(mapsNF_elton_trophiclevel$TrophicLevel) # examine boxplot
...
mapsNF_elton_score <- mapsNF_elton_trophiclevel %>%
  rowwise() %>%
...
mapsNF_elton_score %>% group_by(Diet.score) %>% count()
mapsNF_elton_score %>% group_by(Strata.score) %>% count()

# get final trophic traits data frame by simplifying mapsNF_elton_score
mapsNF_trophic <- mapsNF_elton_score %>% 
...
mapsNF_HWI <- avonet %>%
  ...
  left_join(NFnames_stats, ., by="Species1_BirdLife") # keep all rows in NFnames_stats (our 62 species) and add only the data from avonet for those species

# are any species missing values for HWI?
mapsNF_HWI %>% filter(is.na(Hand.Wing.Index)) %>% nrow() # keep any rows where HWI = NA (aka is missing)
# this should print 0 if all species have values

# examine the distribution of values for HWI
range(mapsNF_HWI$Hand.Wing.Index) # min = 11.4, max = 33.5
hist(mapsNF_HWI$Hand.Wing.Index) # look at a histogram of the values
boxplot(mapsNF_HWI$Hand.Wing.Index)
...
NFspp_allnames # we will use NFspp_names to reduce data

mapsNFspp_morphs <- left_join(NFspp_allnames, dat_10yr) %>%
  filter(AgeCat == "Adult") %>%
...
mapsNF_massHWI <- mapsNFspp_morphs %>%
  group_by(SPEC) %>%
  summarize(Mass=mean(WEIGHT, na.rm=T)) %>%
  right_join(mapsNF_HWI, .)
...
mapsNF_pigot <- pigot %>%
  ...
  left_join(NFnames_stats, ., by="Species3_BirdTree") # join modified pigot traits data frame and NFnames_stats data frame

# combine with mass and HWI to get complete data frame of morphs
mapsNF_morphometrics <- left_join(mapsNF_massHWI, mapsNF_pigot)
...
mapsNF_longevity <- longevity %>% 
  ...
  left_join(NFnames_stats, ., by="Species1_BirdLife") # Join with NF stats by species name column 

# Check if there are missing values for max. longevity 
mapsNF_longevity %>% filter(is.na(Maximum.longevity)) %>% nrow() #There are no missing values 

# Examine max longevity
range(mapsNF_longevity$Maximum.longevity) # The range is 6.10 - 14.49
mean(mapsNF_longevity$Maximum.longevity) # The mean is 9.04 
hist(mapsNF_longevity$Maximum.longevity)
...
mapsNF_clutch_BL <- clutch %>% rename(Species1_BirdLife = scientific_name) %>% # rename columns to match MAPS
  left_join(NFnames_stats, ., by="Species1_BirdLife") %>% filter(!is.na(class))

mapsNF_clutch_BT <- clutch %>% rename(Species3_BirdTree = scientific_name) %>% 
  left_join(NFnames_stats, ., by="Species3_BirdTree") %>% filter(!is.na(class))

mapsNF_clutch <- full_join(mapsNF_clutch_BL, mapsNF_clutch_BT) %>%
  select(SPEC, COMMONNAME, Species1_BirdLife, Species2_eBird, Species3_BirdTree,
         litter_or_clutch_size_n)
nrow(mapsNF_clutch) # all 62 species

# combine longevity and adult survival with clutch traits
mapsNF_lifehistory <-
  left_join(mapsNF_longevity, mapsNF_clutch) 

##########################################################################################
### sexual selection traits #####

### SS trait 1: sexual size dimorphism
# use mapsNFspp_morphs from above
...
mapsNF_sexdimorphism <- mapsNFspp_morphs %>%
...
mapsNF_plumage <- plumage %>%
  ...
  left_join(NFnames_stats, ., by="Species3_BirdTree") %>% # join modified plumage traits data frame and NFnames_stats data frame
  rowwise() %>%
  mutate(Plumage_DC = Male_plumage_score - Female_plumage_score) # calculate dichromatism

# are any missing?
mapsNF_plumage %>% filter(is.na(Plumage_DC))
# Yes. Two species: RBSA and DOWO

# combine with size dimorphism
mapsNF_sizeplumage <- left_join(mapsNF_sexdimorphism, mapsNF_plumage) %>%
...
mapsNF_delhey <- delhey %>%
  ...
  left_join(NFnames_stats, ., by="Species3_BirdTree") %>%
...
write.csv(mapsNF_delhey, here("Data", "mapsNF_delhey.csv")) # save the updated delhey data 
...
# import updated file
mapsNF_matingsystem <- read.csv(here("Data", "mapsNF_delhey_update.csv")) # read in updated delhey data 
# added 3 species to updated file (BUOR, SPTO and WOTH)
# 1 species updated - NOCA
#View(mapsNF_matingsystem)

# how many species per category of sex.sel.m?
mapsNF_matingsystem %>% group_by(sex.sel.m) %>% count()
# most species fall into sex.sel.m = 0 or 1

# how many species per category of sex.sel.f?
mapsNF_matingsystem %>% group_by(sex.sel.f) %>% count()
# most species fall into sex.sel.f = 0 

# are any species missing values?
mapsNF_matingsystem %>% filter(is.na(sex.sel.m))
mapsNF_matingsystem %>% filter(is.na(sex.sel.f))
...
mapsNF_sexualselection <- left_join(mapsNF_sizeplumage, mapsNF_matingsystem)

View(mapsNF_sexualselection) 

# check degree of correlation between some of the variables to make sure they can be used in the same model
mapsNF_SScheck <- mapsNF_sexualselection %>% filter(!is.na(Plumage_DC))

# plumage dichromatism and size dimorphism
set.seed(285)
confintr::ci_cor(mapsNF_SScheck$Plumage_DC, mapsNF_SScheck$Wing_DM, method="spearman", type="bootstrap") # low

# plumage dichromatism and degree of polygyny in males
set.seed(597)
confintr::ci_cor(mapsNF_SScheck$Plumage_DC, mapsNF_SScheck$sex.sel.m, method="spearman", type="bootstrap") # low

# size dimorphism and degree of polygyny in males
set.seed(463)
confintr::ci_cor(mapsNF_SScheck$Wing_DM, mapsNF_SScheck$sex.sel.m, method="spearman", type="bootstrap") # low
...
mapsNF_combo <- left_join(mapsNF_morphometrics, mapsNF_trophic)
set.seed(175)
confintr::ci_cor(mapsNF_combo$Beak_PC2, mapsNF_combo$TrophicLevel, method="spearman", type="bootstrap")
plot(mapsNF_combo$Beak_PC2, mapsNF_combo$TrophicLevel, xlab="Beak PC2", ylab = "Trophic Level")
...
saveRDS(mapsNF_morphometrics, here("Outputs", "mapsNF_morphometrics_m2.rds"))
saveRDS(mapsNF_lifehistory, here("Outputs", "mapsNF_lifehistory_m2.rds"))
saveRDS(mapsNF_trophic, here("Outputs", "mapsNF_trophic_m2.rds"))
saveRDS(mapsNF_sexualselection, here("Outputs", "mapsNF_sexualselection_m2.rds"))
